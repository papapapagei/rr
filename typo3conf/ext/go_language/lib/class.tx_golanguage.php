...
/**
 * Libary 'Automatic language configuration' for the 'go_language' extension.
 *
 * @author	Caspar Stuebs <caspar@gosign.de>
 * @package	TYPO3
 * @subpackage	tx_golanguage
 */
class tx_golanguage implements t3lib_Singleton {
	public $prefixId      = 'tx_golanguage';		// Same as class name
	public $scriptRelPath = 'lib/class.tx_golanguage.php';	// Path to this script relative to the extension dir.
	...
	function __construct() {
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['go_language'])) {
			$this->extensionConfig = array_merge($this->extensionConfig, $GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['go_language']);
		}
		else {
			$tempExtensionConfig = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['go_language']);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/go_language/lib/class.tx_golanguage.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/go_language/lib/class.tx_golanguage.php']);
}
