	...
	************************************************************************************************************/	
	...
	//alert('ich werde ausgeführt: image-crop.js');
	...
	function crop_createDivElements()
	{
		...
		smallSquare_tl.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_tr.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_bl.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_br.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_tc.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_bc.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_lc.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
		...
		smallSquare_rc.src = '../typo3conf/ext/go_imageedit_be/res/image-crop/images/small_square.gif';
	...
	function cropScript_mouseMove(e)
	{
		...
		// = Math.round((cropDiv_dotted.style.width.replace('px','')/1 + (cropToolBorderWidth*2)) * (crop_originalImageWidth/crop_imageWidth));
		//.value = Math.round((cropDiv_dotted.style.height.replace('px','')/1 + (cropToolBorderWidth*2)) * (crop_originalImageHeight/crop_imageHeight));		
		
			
			if(!crop_script_fixedRatio && !e.ctrlKey)cropWidthRatio = (document.getElementById('input_crop_width').value)/(document.getElementById('input_crop_height').value);
		}
		...
		if(updateFormValuesAsYouDrag)cropScript_updateFormValues(true);
	...
	function cropScript_updateFormValues(execute)
	{
		document.getElementById('input_crop_x').value = Math.round(cropDiv_dotted.style.left.replace('px','')/1 * (crop_originalImageWidth/crop_imageWidth));
		document.getElementById('input_crop_y').value = Math.round(cropDiv_dotted.style.top.replace('px','')/1 * (crop_originalImageHeight/crop_imageHeight));
		...
		/*
		alert(document.getElementById('input_crop_width').value);
		var prop = document.getElementById('input_crop_width').value/document.getElementById('input_crop_height').value;
		if (document.getElementById('input_crop_width').value <= 400)
		{
			document.getElementById('input_crop_width').value = 400;
		}
		document.getElementById('input_crop_height').value = document.getElementById('input_crop_height').value * prop;
		*/
		
		
		if (execute)
		{
			saveToMysqlDB(document.getElementById('input_crop_x').value,document.getElementById('input_crop_y').value,document.getElementById('input_crop_width').value,document.getElementById('input_crop_height').value);
		}
	}
	
	function saveToMysqlDB(oX, oY, w, h)
	{
		new Ajax.Request('../typo3conf/ext/go_imageedit_be/class.imageedit.php', {
		method: 'get',
		parameters:  {"uid":pageuid, "action":"saveImageValues",'oX':oX*zoom, 'oY':oY*zoom, 'w':w*zoom, 'h':h*zoom, 'imgname':imgname},
	...
	function cropScript_stopResizeMove()
	{
		...
		crop_moveCounter = -1;
		cropDiv_dotted.style.cursor = 'move';
		cropScript_updateFormValues(true);
		if(document.all){
			var div = cropDiv_dotted.getElementsByTagName('DIV')[0];
	...
	function cropScript_setCropSizeByInput()
	{
		...
		//if(obj_width.value.length==0) obj_width.value=100;
		if(obj_width.value.length==0) obj_width.value=dragWidth;
		//if(obj_height.value.length==0) obj_height.value=100;
		if(obj_height.value.length==0) obj_height.value=dragHeight;
	...
	function init_imageCrop()
	{
		...
		crop_progressBar();
			...
			//}
		cropScript_setCropSizeByInput();
		cropScript_updateFormValues(false);
	}